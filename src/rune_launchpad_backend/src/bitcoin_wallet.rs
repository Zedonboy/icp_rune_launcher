...
use crate::{bitcoin_api, ecdsa_api::{self, sign_with_ecdsa}, types::{self, Converter, RuneInfo}};
// use bitcoin::util::psbt::serialize::Serialize;
use bitcoin::{
    absolute::LockTime, blockdata::{script::Builder, witness::Witness}, consensus::serialize, hashes::Hash, script::{self, PushBytes, PushBytesBuf}, sighash::{EcdsaSighashType, SighashCache}, Address, AddressType, OutPoint, Script, Transaction, TxIn, TxOut, Txid
};
...
const RUNE_ISSUANCE_COST : u64 = 546;

//Extending Rune  to non-async
...
pub async fn withdraw(
    ...
    let transaction = build_withdraw_transaction(&own_public_key, &own_address, &own_utxos, &dst_address, amount, fee_per_byte)
    .await;
...
pub async fn etch_rune(
    ...
    // Build the transaction that sends `amount` to the destination address.
    let transaction = build_etch_transaction(
        &own_public_key,
        &own_address.clone().assume_checked(),
        &own_utxos,
        &dst_address.assume_checked(),
        fee_per_byte,
        info,
...
async fn build_withdraw_transaction(
    ...
    loop {
        let transaction =
            build_withdraw_transaction_with_fee(own_utxos, own_address, dst_address, amount, total_fee).await
        ...
        let signed_tx_bytes_len = signed_transaction.size() as u64;
...
// destination address.
async fn build_etch_transaction(
    own_public_key: &[u8],
    own_address: &Address,
    own_utxos: &[Utxo],
    dst_address: &Address,
    ...
    // and sign a transaction, see what its size is, and then update the fee,
    // rebuild the transaction, until the fee is set to the correct amount.
    print("Building Rune Etch transaction...");
    let mut total_fee = 0;
    loop {
        let transaction =
            build_etch_transaction_with_fee(own_utxos, own_address, dst_address, RUNE_ISSUANCE_COST, total_fee, info.clone()).await
                .expect("Error building transaction.");
        ...
        let signed_transaction = sign_transaction(
            ...
            own_address,
            transaction.clone(),
            key_name.clone(), // mock key name
            derivation_path.clone(),           // mock derivation path
            sign_with_ecdsa,
        )
        .await;
...
async fn build_etch_transaction_with_fee(
    own_utxos: &[Utxo],
    own_address: &Address,
    dst_address: &Address,
    amount: u64, // etching amount. its 546
    fee: u64,
    info : RuneInfo
    ...
    let to = bitcoin::address::Address::from_str(dst_address.to_string().as_str()).unwrap().assume_checked();
    let converter = Converter{base_offset : 1};
    let issuance_output = Rune::issuance_outputs(converter.symbol_to_int(&info.symbol).unwrap(), info.decimals, info.amount, to).await.unwrap();
    let mut outputs = issuance_output;
...
async fn build_withdraw_transaction_with_fee(
    own_utxos: &[Utxo],
    own_address: &Address,
    dst_address: &Address,
    amount: u64,
    ...
    let mut outputs = vec![TxOut {
        ...
        value: amount - fee, //sending a reduced amount.
    }];

    let remaining_amount = total_spent - amount;
    ...
    Fut: std::future::Future<Output = Vec<u8>>,
{
    ...
    for (index, input) in transaction.input.iter_mut().enumerate() {
        let sig_hash_cache = SighashCache::new(txclone.clone());
        
        let sighash =
        sig_hash_cache.legacy_signature_hash(index, &own_address.script_pubkey(), SIG_HASH_TYPE.to_u32()).unwrap();
        ...
        let buf = PushBytesBuf::try_from(sig_with_hashtype).unwrap();
        let script_buf = PushBytesBuf::try_from(own_public_key.to_vec()).unwrap();
        input.script_sig = Builder::new()
            .push_slice(buf)
            .push_slice(script_buf)
            .into_script();
        input.witness.clear();
    }
